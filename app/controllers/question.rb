RubyConans::App.controllers :question do
  get :index, map: '/questions/:language', provides: [:json] do
    questions = Question.where(language: params[:language].downcase).order(:display_order)
    questions.to_json
  end

  get :with_topic, map: '/questions/:language/:topic', provides: [:json] do
  ...
  post :answer, with: :question_id, provides: [:json] do
